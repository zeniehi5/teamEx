...
<script type="text/javascript">
	...
	function saveForm(){
		...
		var inputFile = $("input[name='uploadFile']").val();
		var fileArr = inputFile.split("\\")
		const fileName =fileArr[fileArr.length-1]
		var files = $("input[name='uploadFile']")[0].files[0];
...
<jsp:include page="/WEB-INF/partner/header.jsp"/>	
	<div class="main">
		...
		<div class="body">
			<div class="gallery">
				...
				<div class="upload__box">
					...
					<div class="upload__btn-box">
						<div class="pics">
							<select id="roomType" name="roomType" onchange="saveRoomType();">
							...
							<label class="upload__btn">
								<input id="image" class="upload__inputfile" type="file" name="uploadFile" value="사진 추가" multiple="multiple">
									<span>사진추가</span>
							</label>
						</div>
					</div>	
					...
					<div class="uploaded_img">
						...
						<div>
							<div id="popup">
								<h3>${pic.file_name}</h3><br>
								<img src="${pic.file_url}"/>
							</div>
							<div style="display: flex; gap: 20px; margin-top: 20px; flex-wrap: wrap;">
								<c:forEach items="${room_image}" var="UpdateImageVO" varStatus="vs">
									<div id="pic${vs.index}" onclick="openPicModal()">
										<h3>${UpdateImageVO.type}</h3><br>
										<img src="${UpdateImageVO.file_url}">
									</div>
								</c:forEach>
							</div>
						</div>
			...
			<div class="bottom">
				<button type="button" id="continue" onclick="saveForm()">저장</button>
			</div>
		</div>
			...
			<form id="deleteForm" action="remove-picture.pdo?serialnumber=${UpdateImageVO.serialnumber}&&file_name=${UpdateImageVO.file_name}" method="POST">
				<div class="modal">
					<div class="explain">
						<div class="change-pic">
							<div class="change-pic-1">
								<svg data-test-id="default-icon" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 24 24">
					...
					<div class="modal-bottom">
						<div class="modal_pic">
							<h3>${pic.file_name}</h3>
							<img src="${pic.file_url}" name="file_url">
							<input type="hidden" value="${UpdateImageVO.serialnumber}">
							<input type="hidden" value="${UpdateImageVO.file_name}">
						</div>
						<div>
							<button type="button" class="button button_primary" id="delete" onclick="deletePic()">삭제</button>
						</div>	
					</div>
					<div class="close">
						<button type="button" class="button button_primary" id="close">닫기</button>
					</div>
				</div>
			</form>
		</div>
